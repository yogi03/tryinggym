...
export async function sendOTP(email: string): Promise<{ success: boolean; error?: string }> {
  try {
    // 1. Check the email belongs to a registered trainer account
    const trainerRef = adminDb.collection("trainerAccounts");
    const snapshot = await trainerRef.where("email", "==", email).get();
 
    if (snapshot.empty) {
      return { success: false, error: "No trainer account found with this email address." };
    }
    ...
    await trainerDoc.ref.update({ otp, otpExpiresAt: expiresAt });
 
    await sendEmail({
      to: email,
      subject: "Your Trainer Password Reset OTP",
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 480px; margin: 0 auto; padding: 24px; border: 1px solid #e5e7eb; border-radius: 8px;">
          <h2 style="color: #1f2937; margin-bottom: 8px;">Trainer Password Reset</h2>
          <p style="color: #6b7280; margin-bottom: 24px;">Use the code below to reset your trainer password. This code is valid for <strong>60 seconds</strong>.</p>
...
export async function verifyOTP(
  ...
  try {
    ...
    await trainerDoc.ref.update({ otp: null, otpExpiresAt: null });
