...
export async function sendOTP(email: string): Promise<{ success: boolean; error?: string }> {
  try {
    // 1. Check the email belongs to a registered admin
    ...
    // 3. Store OTP in Firestore (under the admin's document)
    const adminDoc = snapshot.docs[0];
    await adminDoc.ref.update({ otp, otpExpiresAt: expiresAt });
    ...
    await sendEmail({
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 480px; margin: 0 auto; padding: 24px; border: 1px solid #e5e7eb; border-radius: 8px;">
          ...
          <p style="color: #6b7280; margin-bottom: 24px;">Use the code below to reset your admin password. This code is valid for <strong>60 seconds</strong>.</p>
...
export async function verifyOTP(
  ...
  try {
    ...
    await adminDoc.ref.update({ otp: null, otpExpiresAt: null });
